"""Stable residual VQ for SeisDAC training.

Same architecture as ``dac.nn.quantize``, but commitment/codebook losses are
computed on L2-normalized latents so the loss scale matches the cosine-style
codebook lookup. This avoids VQ loss explosions when encoder activations grow
large.
...
def _normalized_vq_losses(z_e: torch.Tensor, z_q: torch.Tensor):
    """VQ-VAE losses in the same normalized space used for nearest-neighbor lookup."""
    z_e_norm = F.normalize(z_e, dim=1, eps=_NORM_EPS)
    z_q_norm = F.normalize(z_q, dim=1, eps=_NORM_EPS)

    commitment_loss = F.mse_loss(z_e_norm, z_q_norm.detach(), reduction="none").mean([1, 2])
    codebook_loss = F.mse_loss(z_q_norm, z_e_norm.detach(), reduction="none").mean([1, 2])
    return commitment_loss, codebook_loss


class VectorQuantizeStable(nn.Module):
    """Vector quantizer with scale-stable normalized VQ losses."""
    ...
    def decode_latents(self, latents, *, return_distances: bool = False):
        ...
        encodings = F.normalize(encodings, eps=_NORM_EPS)
        codebook = F.normalize(codebook, eps=_NORM_EPS)
...
class ResidualVectorQuantizeStable(nn.Module):
    """Residual VQ stack using :class:`VectorQuantizeStable`."""
    ...
    def __init__(
        ...
        self.quantizers = nn.ModuleList(
            [
                VectorQuantizeStable(input_dim, codebook_size, codebook_dim[i])
